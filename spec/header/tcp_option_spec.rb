  ...
  module Header
    class TCP
      describe Option do
        ...
        describe '#read' do
          it 'reads an integer Option' do
            ...
            expect(opt.length).to eq(6)
            expect(opt.value).to eq(PacketGen.force_binary "\x00\x00\x00\x01")
          end
      ...
      describe NOP do
        ...
        it 'is a single byte option' do
          expect(NOP.new.to_s).to eq(PacketGen.force_binary "\x01")
        end
      end
      ...
      describe MSS do
        ...
        it 'is a 4-byte option' do
          mss = MSS.new
          expect(mss.to_s).to eq(PacketGen.force_binary "\x02\x04\x00\x00")
          mss.value = 0x123
          expect(mss.to_s).to eq(PacketGen.force_binary "\x02\x04\x01\x23")
        end
      end
      ...
      describe WS do
        ...
        it 'is a 3-byte option' do
          ws = WS.new
          expect(ws.to_s).to eq(PacketGen.force_binary "\x03\x03\x00")
          ws.value = 0x12
          expect(ws.to_s).to eq(PacketGen.force_binary "\x03\x03\x12")
        end
      end
      ...
      describe SACKOK do
        ...
        it 'is a 2-byte option' do
          expect(SACKOK.new.to_s).to eq(PacketGen.force_binary "\x04\x02")
        end
      end
      ...
      describe SACK do
        ...
        it 'is a multi-byte option' do
          sack = SACK.new
          expect(sack.to_s).to eq(PacketGen.force_binary "\x05\x02")
          1.upto(12) do |i|
            sack.value = 'z' * i
            sack.length = 2 + i
            expected = [5, 2 + i].pack('C*') + 'z' * i
            expect(sack.to_s).to eq(PacketGen.force_binary expected)
          end
        end
      end
      ...
      describe ECHO do
        ...
        it 'is a 6-byte option' do
          echo = ECHO.new
          expect(echo.to_s).to eq(PacketGen.force_binary "\x06\x06\x00\x00\x00\x00")
          echo.value = 0xff010203
          expect(echo.to_s).to eq(PacketGen.force_binary "\x06\x06\xff\x01\x02\x03")
        end
      end
      ...
      describe ECHOREPLY do
        ...
        it 'is a 6-byte option' do
          echor = ECHOREPLY.new
          expect(echor.to_s).to eq(PacketGen.force_binary "\x07\x06\x00\x00\x00\x00")
          echor.value = 0xff010203
          expect(echor.to_s).to eq(PacketGen.force_binary "\x07\x06\xff\x01\x02\x03")
        end
      end
      ...
      describe TS do
        ...
        it 'is a 10-byte option' do
          ts = TS.new
          expect(ts.to_s).to eq(PacketGen.force_binary("\x08\x0a" + "\x00" * 8))
          ts.value = "\x7f" + "\x01" * 7
          expect(ts.to_s).to eq(PacketGen.force_binary("\x08\x0a\x7f" + "\x01" * 7))
        end
      end
    end
  end
